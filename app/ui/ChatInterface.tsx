...
import DigitalAvatarSidebar from './DigitalAvatarSidebar';
...
export default function ChatInterface() {
  const { messages, isLoading, sendMessage, removeMessagesFromIndex } =
    useChat();
  ...
  const [isAvatarSidebarOpen, setIsAvatarSidebarOpen] = useState(false);
  ...
  const handleHistoryLoaded = (records: ChatHistoryRecord[]) => {
    // 将历史记录转换为消息格式
    const convertedMessages: HistoryMessage[] = records.map(
      (record, index) => ({
        id: `history-${index}`,
  ...
  const handleAvatarClick = () => {
    setIsAvatarSidebarOpen(!isAvatarSidebarOpen);
  };

  const handleAvatarSidebarClose = () => {
    setIsAvatarSidebarOpen(false);
  };

  return (
    <div className="flex h-screen">
      {/* 数字人侧边栏 */}
      <DigitalAvatarSidebar
        isOpen={isAvatarSidebarOpen}
        onClose={handleAvatarSidebarClose}
      />
      ...
      <div
        ...
        style={{
          marginLeft: !isMobile && isAvatarSidebarOpen ? '400px' : '0',
        }}
      >
        {/* 固定的顶栏 */}
